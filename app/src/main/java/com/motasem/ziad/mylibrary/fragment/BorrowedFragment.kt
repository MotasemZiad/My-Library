...
class BorrowedFragment : Fragment(), BooksAdapter.onClick {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        data = db.getAllBorrowed()
        rvBorrowed.layoutManager = LinearLayoutManager(activity)
        rvBorrowed.setHasFixedSize(true)
        val favorateAdapter = CategoriesAdapter(activity!!, data, this)
        rvBorrowed.adapter = favorateAdapter
        super.onActivityCreated(savedInstanceState)

        val swipeToDeleteCallback = object : SwipeToDeleteCallback(activity!!) {
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                val adapter: CategoriesAdapter = rvBorrowed.adapter as CategoriesAdapter
        ...
        itemTouchHelper.attachToRecyclerView(rvBorrowed)
    }
